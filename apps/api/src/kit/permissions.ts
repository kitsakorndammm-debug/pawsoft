 ...
 *
 * **ห้าม grant ให้บทบาทอื่นเลย** (ผู้ใช้ตัดสิน 2026-09-18: "ซ่อนออกหมด ติ๊กให้บทบาทอื่น
 * ไม่ได้เลย") — บทบาทระบบ (`admin`) เห็นเสมอเพราะถือทุก key โดยไม่ผ่าน `role_permission`
 * (ดู `permissionsOf`) ส่วนบทบาทอื่นไม่มีทางได้ key นี้เลย เพราะมันไม่โผล่ในหน้าติ๊กสิทธิ์
 * ให้เลือก — ดู `NOT_GRANTABLE` และ `permissionRoutes` ที่กรองออกตอนส่งลิสต์ให้หน้าเว็บ
 */
...
/**
 * key ที่ไม่ให้ขึ้นในหน้า "ติ๊กสิทธิ์" ของบทบาทไหนเลย — ต่างจาก key อื่นที่ admin
 * เลือก grant ให้บทบาทอื่นได้ตามต้องการ
 *
 * **ตัวการ์ด (`guardAuditLogRead`) ยังตรวจ key นี้เหมือนเดิมทุกอย่าง** — ที่ซ่อนคือ
 * "ทางที่จะได้ key มา" ไม่ใช่ตัวการ์ด เพราะไม่มี `role_permission` แถวไหนชี้มาที่นี่ได้เลย
 * (หน้าติ๊กไม่มีให้เลือก) บทบาทอื่นจึงไม่มีทางมี key นี้ไม่ว่าแอดมินจะกดอะไร
 */
export const NOT_GRANTABLE: ReadonlySet<string> = new Set([AUDIT_LOG_PERMISSION.read])
