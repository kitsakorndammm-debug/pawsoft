import { inUse, notFound } from '../../kit/app-error.ts'
import { diffFields, writeAudit } from '../../kit/audit.ts'
import { db, inTx, type Db, type Tx } from '../../kit/db.ts'
import { asDuplicate } from '../../kit/duplicate.ts'
import { closeGap, resolveMoveTarget, shiftBetween, shiftDownForNew,
  lockSortList,
} from '../../kit/sort-order.ts'
import { cleanRequired, literal } from '../../kit/text.ts'
import type { Species } from '../../../prisma/generated/client.ts'

/**
 * ชนิดสัตว์ — หมา แมว กระต่าย
 *
 * **เป็นตาราง ไม่ใช่ enum** เพราะคลินิกที่เริ่มรับสัตว์แปลกใหม่ต้องเพิ่มเองได้ โดยไม่ต้อง
 * แก้โค้ดแล้ว migrate ฐาน · ต่างจาก `PetSex` ที่ชุดค่าปิดตายจริง
 *
 * รูปเดียวกับทะเบียนอื่นที่ลากเรียงได้ ต่างตรงเดียว: **มีลูกสองชั้นชี้มา** — สัตว์ชี้ตรง
 * และสายพันธุ์ชี้มาด้วย · การลบจึงต้องถามถึงทั้งสองฝั่ง
 */

const MODULE = 'species'
const LABEL = 'ชนิดสัตว์'
const NAME_MAX = 200

export const SPECIES_LIST_LIMIT = 500

const name = (raw: string) =>
  cleanRequired(raw, { field: 'name', label: `ชื่อ${LABEL}`, max: NAME_MAX })

function refuseDuplicate(e: unknown, value: string): never {
  return asDuplicate(e, { message: `มี${LABEL}ชื่อนี้อยู่แล้ว`, field: 'name', value })
}

/**
 * ลูกที่ยังใช้งานอยู่กันการลบไว้ — พร้อมบอกว่ามีกี่ตัว
 *
 * ตัวเลขคือสิ่งที่เปลี่ยน "ลบไม่ได้" ให้เป็นอะไรที่ผู้ใช้ทำต่อได้ เขารู้ว่าต้องย้ายอีกเท่าไหร่
 */
async function guardDelete(id: bigint, at: Tx | Db): Promise<void> {
  const [pets, breeds] = await Promise.all([
    at.pet.count({ where: { speciesId: id, deletedAt: null } }),
    at.breed.count({ where: { speciesId: id, deletedAt: null } }),
  ])

  if (pets > 0) throw inUse(`ลบ${LABEL}นี้ไม่ได้ เพราะยังมีสัตว์เป็นชนิดนี้อยู่`, { pets })

  /**
   * **สายพันธุ์กันการลบไว้ด้วย และมันบังคับแบบ "ต้องลบเอง" ไม่ใช่ลบตามให้**
   *
   * สายพันธุ์ที่ลอยอยู่โดยไม่มีชนิด คือแถวที่เลือกไม่ได้และมองไม่เห็น · ส่วนการลบตาม
   * ให้เงียบ ๆ แปลว่าคนกดลบ "แมว" หายไป 40 สายพันธุ์โดยไม่มีใครบอก
   */
  if (breeds > 0) {
    throw inUse(`ลบ${LABEL}นี้ไม่ได้ เพราะยังมีสายพันธุ์อยู่ใต้มัน — ลบสายพันธุ์ก่อน`, { breeds })
  }
}

/**
 * **ไม่มีตัวปลดลูก ต่างจากหมวดยา** — และนั่นเป็นเพราะรูปของ FK ต่างกัน
 *
 * `drug.categoryId` เป็น nullable ปลดได้ · แต่ `pet.speciesId` กับ `breed.speciesId`
 * บังคับ ปลดไม่ได้โดยไม่ลบแถวทิ้ง · `guardDelete` จึงปฏิเสธไปตั้งแต่ยังมีลูกที่ยังอยู่
 * และลูกที่ตายแล้วก็ชี้ค้างไว้ได้ ไม่มีใครอ่านมันอีก
 */

export type ListSpeciesRowsInput = {
  q?: string | undefined
  limit?: number | undefined
}

/**
 * ดูรายการ — ทั้งลิสต์ ไม่แบ่งหน้า
 *
 * `orderBy` มีสองคีย์เพราะแถวที่ถือ `sortOrder` เท่ากันไม่มีลำดับที่แน่นอนถ้าไม่มีคีย์ที่สอง
 * และ Postgres คืนมาสลับกันได้ระหว่างสองครั้งที่เรียก — ลิสต์จะดูเหมือนสลับตัวเองทุกครั้ง
 * ที่ refresh · `id: 'desc'` ทำให้แถวที่เพิ่งสร้างขึ้นบน และไม่ขึ้นกับว่าฐานเรียงภาษาไทยยังไง
 */
export async function listSpeciesRows(
  input: ListSpeciesRowsInput = {},
  tx?: Tx,
): Promise<Species[]> {
  const q = input.q?.trim()
  const limit = Math.min(input.limit ?? SPECIES_LIST_LIMIT, SPECIES_LIST_LIMIT)

  return (tx ?? db).species.findMany({
    where: {
      deletedAt: null,
      ...(q ? { name: { contains: literal(q) } } : {}),
    },
    orderBy: [{ sortOrder: 'asc' }, { id: 'desc' }],
    take: limit,
  })
}

export type CreateSpeciesInput = { name: string }

/**
 * เพิ่มรายการ — แถวใหม่ขึ้นบนสุดเสมอ
 *
 * ทั้งก้อนอยู่ในทรานแซกชันเดียว · วางแถวไว้ที่ลำดับ 0 แปลว่าทุกแถวที่เหลือต้องเลื่อนลง
 * ทำนอกทรานแซกชันเมื่อไหร่ สองคนที่กดสร้างพร้อมกันจะเลื่อนทั้งคู่แล้วเขียนลงที่ 0 ทั้งคู่
 */
export async function createSpecies(
  input: CreateSpeciesInput,
  actorId: bigint,
  outerTx?: Tx,
): Promise<Species> {
  const value = name(input.name)

  try {
    return await inTx(outerTx, async (tx) => {
      // เข้าคิวก่อนทำงาน — เหตุผลอยู่ที่ `lockSortList`
      await lockSortList({ tx, name: 'species' })
      await shiftDownForNew({ tx, table: tx.species, name: 'species' })

      const created = await tx.species.create({
        data: { name: value, sortOrder: 0, createdBy: actorId, updatedBy: actorId },
      })

      await writeAudit(tx, {
        action: `${MODULE}.create`,
        module: MODULE,
        recordId: created.id,
        after: { name: created.name, sortOrder: created.sortOrder },
        userId: actorId,
      })

      return created
    })
  } catch (e) {
    refuseDuplicate(e, value)
  }
}

export type UpdateSpeciesInput = { name: string }

/** แก้ชื่ออย่างเดียว — ลำดับเป็นงานของ `moveSpecies` */
export async function updateSpecies(
  id: bigint,
  input: UpdateSpeciesInput,
  actorId: bigint,
  outerTx?: Tx,
): Promise<Species> {
  const value = name(input.name)

  // findFirst ไม่ใช่ findUnique — แถวต้องยังมีชีวิต และ `deletedAt` ไม่ได้อยู่ในคีย์ไหน
  const existing = await (outerTx ?? db).species.findFirst({
    where: { id, deletedAt: null },
  })
  if (!existing) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })

  const changed = diffFields({ name: existing.name }, { name: value })

  try {
    return await inTx(outerTx, async (tx) => {
      const updated = await tx.species.update({
        where: { id },
        data: { name: value, updatedBy: actorId },
      })

      // ฟอร์มแก้ข้อมูลส่งทุกฟิลด์กลับมาตอนกดบันทึก การกดบันทึกโดยไม่แก้อะไรจึงเป็น
      // เรื่องปกติ · log ที่บอกว่าไม่มีอะไรเปลี่ยน คือขยะในตารางเดียวที่ต้องอ่านออก
      if (changed.after !== null) {
        await writeAudit(tx, {
          action: `${MODULE}.update`,
          module: MODULE,
          recordId: id,
          before: changed.before,
          after: changed.after,
          userId: actorId,
        })
      }

      return updated
    })
  } catch (e) {
    refuseDuplicate(e, value)
  }
}

/**
 * ลบ — soft delete แล้วปิดช่องที่มันทิ้งไว้
 *
 * **ลบตัวที่ลบไปแล้วไม่ error แต่ไม่ทำอะไรเลย** — การลบเกิดครั้งเดียว การกดปุ่มสิบครั้ง
 * ไม่ควรอ่านเป็นการลบสิบครั้ง
 */
export async function deleteSpecies(
  id: bigint,
  actorId: bigint,
  outerTx?: Tx,
): Promise<void> {
  const existing = await (outerTx ?? db).species.findUnique({ where: { id } })
  if (!existing) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })

  if (existing.deletedAt !== null) return

  // ปฏิเสธก่อนเปิดทรานแซกชัน ถ้ายังมีใครสังกัดอยู่
  await guardDelete(id, outerTx ?? db)

  await inTx(outerTx, async (tx) => {
    // เข้าคิวก่อนทำงาน — เหตุผลอยู่ที่ `lockSortList`
    await lockSortList({ tx, name: 'species' })
    /**
     * อ่าน `sortOrder` **ใหม่หลังจับกุญแจ** ไม่ใช่ใช้ค่าที่อ่านมาตอนตรวจว่าแถวมีจริง
     *
     * ค่าที่อ่านก่อนเข้าคิวคือค่าก่อนที่คนข้างหน้าจะเลื่อนลิสต์ · ลบสิบแถวพร้อมกันแล้ว
     * ทุกคนอ่านค่าเดิมชุดเดียวกัน แล้วต่างคนต่างปิดช่องจากลำดับที่ไม่มีอยู่แล้ว
     * ผลคือสองแถวถือเลขเดียวกัน โดยไม่มี error สักตัว (วัดจริง 2026-08-26)
     */
    const current = await tx.species.findUnique({ where: { id } })
    if (!current || current.deletedAt !== null) return
    const vacated = current.sortOrder

    await tx.species.update({
      where: { id },
      data: {
        deletedAt: new Date(),
        deletedBy: actorId,
        // `updatedBy` ไม่ถูกแตะ — `deletedBy` บอกอยู่แล้วว่าใครลบ ทับอีกตัวไปด้วย
        // คือลบร่องรอยว่าใครแก้ข้อมูลแถวนี้เป็นคนสุดท้าย คนละคำถามกัน
        //
        // null ไม่ใช่ตัวเลข: แถวไม่ได้อยู่ในลิสต์แล้ว จึงไม่ถือลำดับในลิสต์
        sortOrder: null,
      },
    })

    await closeGap({ tx, table: tx.species, name: 'species' }, vacated)

    await writeAudit(tx, {
      action: `${MODULE}.delete`,
      module: MODULE,
      recordId: id,
      before: { name: existing.name, sortOrder: vacated },
      after: null,
      userId: actorId,
    })
  })
}

export type MoveSpeciesInput = {
  /** `null` แปลว่าไปล่างสุด · ไม่ส่งมาเลยแปลว่าคนเรียกไม่ได้บอกว่าจะย้ายไปไหน */
  beforeId: bigint | null
}

/** ย้ายลำดับ — เลขคณิตอยู่ใน `@kit/sort-order` เพราะทุกทะเบียนเดินชุดเดียวกัน */
export async function moveSpecies(
  id: bigint,
  input: MoveSpeciesInput,
  actorId: bigint,
  outerTx?: Tx,
): Promise<void> {
  const at = outerTx ?? db

  const row = await at.species.findFirst({ where: { id, deletedAt: null } })
  if (!row) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })

  const from = row.sortOrder
  if (from === null) {
    // แถวที่ยังอยู่ต้องถือลำดับเสมอ — มาถึงตรงนี้แปลว่าตารางไม่คงเส้นคงวา
    throw notFound(`${LABEL}นี้ไม่มีลำดับในลิสต์`, { id: String(id) })
  }

  const to = await resolveMoveTarget(at.species, {
    id,
    from,
    beforeId: input.beforeId,
    label: LABEL,
  })

  // อยู่ตรงนั้นอยู่แล้ว หรือลากลงบนตัวเอง — ไม่มีอะไรขยับ ไม่มีอะไรให้บันทึก
  if (to === null) return

  await inTx(outerTx, async (tx) => {
    // เข้าคิวก่อนทำงาน — เหตุผลอยู่ที่ `lockSortList`
    await lockSortList({ tx, name: 'species' })
    await shiftBetween({ tx, table: tx.species, name: 'species' }, { from, to })

    await tx.species.update({ where: { id }, data: { sortOrder: to, updatedBy: actorId } })

    await writeAudit(tx, {
      action: `${MODULE}.move`,
      module: MODULE,
      recordId: id,
      before: { sortOrder: from },
      after: { sortOrder: to },
      userId: actorId,
    })
  })
}
