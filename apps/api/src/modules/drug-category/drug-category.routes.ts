...
import {
  ...
  moveDrugCategory,
  updateDrugCategory,
} from './drug-category.service.ts'
import type { DrugCategory } from '../../../prisma/generated/client.ts'

/**
 * `/api/departments` — หมวดยา
 *
 * กฎที่เป็นของหมวดยาเอง (ลบไม่ได้ถ้ายังมียาหรือยาสังกัด) อยู่ที่ชั้น service
 ...
 */
function toWire(row: DrugCategory) {
  return {
    id: Number(row.id),
    name: row.name,
...
export const drugCategoryRoutes = new Elysia({ prefix: '/api/drug-categories' })
  .get(
    '/',
    async ({ query, request }) => {
      refuseUnknownQuery(request.url, LIST_QUERY_KEYS)

      const rows = await listDrugCategorys({ q: query.q })
      return ok(rows.map(toWire))
    },
    {
      query: t.Object({ q: t.Optional(t.String()) }),
      beforeHandle: guardRead,
      detail: { summary: 'ดูรายการหมวดยา', tags: ['หมวดยา'] },
    },
  )
  /**
   * `/lookup` — สำหรับ combobox ในหน้าอื่น
   *
   * คืนแค่ `id` กับ `name` · หน้าที่ต้องให้คนเลือกหมวดยาไม่ควรจ่ายค่าขนส่งของคอลัมน์ที่
  ...
  .get(
    ...
    async ({ request }) => {
      ...
      const rows = await listDrugCategorys()
      return ok(rows.map((row) => ({ id: Number(row.id), name: row.name })))
    },
    {
      beforeHandle: guardSignedIn,
      detail: { summary: 'หมวดยาสำหรับ combobox', tags: ['หมวดยา'] },
    },
  )
  .post(
    '/',
    async (ctx) => {
      ...
      const row = await createDrugCategory({ name: ctx.body.name }, userId)
    ...
    },
    {
      ...
      detail: { summary: 'เพิ่มหมวดยา', tags: ['หมวดยา'] },
    },
  )
  .patch(
    '/:id',
    async (ctx) => {
      const { userId } = await getActor(ctx)
      const row = await updateDrugCategory(parseId(ctx.params.id), { name: ctx.body.name }, userId)
    ...
    },
    {
      body: t.Object({ name: t.String() }),
      transform: [guardWrite, refuseUnknownFields(NAME_ONLY)],
      detail: { summary: 'แก้ไขหมวดยา', tags: ['หมวดยา'] },
    },
  )
  /**
  ...
  .patch(
    ...
    },
    {
      ...
      detail: { summary: 'ย้ายลำดับหมวดยา', tags: ['หมวดยา'] },
    },
  )
  .delete(
    '/:id',
    async (ctx) => {
      const { userId } = await getActor(ctx)
      await deleteDrugCategory(parseId(ctx.params.id), userId)

      return ok(null)
    },
    {
      transform: guardWrite,
      detail: { summary: 'ลบหมวดยา', tags: ['หมวดยา'] },
    },
  )
