...
import type { DrugCategory } from '../../../prisma/generated/client.ts'

/**
 * หมวดยา — ฝ่ายผลิต ฝ่ายขาย ฝ่ายบัญชี
 *
 * รูปเดียวกับทะเบียนอื่น ต่างตรงเดียว: **มีคนสังกัดอยู่** ยากับยาชี้มาที่นี่
 * การลบจึงต้องถามถึงพวกเขาก่อน (ผู้ใช้ตัดสิน 2026-08-26 · ดู `///` บนหัว `DrugCategory`)
 */

const MODULE = 'drugCategory'
const LABEL = 'หมวดยา'
const NAME_MAX = 200

export const DRUG_CATEGORY_LIST_LIMIT = 500
...
async function guardDelete(id: bigint, at: Tx | Db): Promise<void> {
  const children = await at.drug.count({ where: { categoryId: id, deletedAt: null } })

  if (children > 0) {
    throw inUse(`ลบหมวดยานี้ไม่ได้ เพราะยังมียาสังกัดอยู่`, { children })
  }
}

/**
 * ลูกที่ถูกลบไปแล้วเลิกชี้มาที่หมวดยาที่กำลังจะหายไป
 *
...
export type ListDrugCategorysInput = {
  q?: string | undefined
 ...
 */
export async function listDrugCategorys(
  input: ListDrugCategorysInput = {},
  tx?: Tx,
): Promise<DrugCategory[]> {
  const q = input.q?.trim()
  const limit = Math.min(input.limit ?? DRUG_CATEGORY_LIST_LIMIT, DRUG_CATEGORY_LIST_LIMIT)

  return (tx ?? db).drugCategory.findMany({
    where: {
      deletedAt: null,
...
export type CreateDrugCategoryInput = { name: string }
...
export async function createDrugCategory(
  ...
  try {
    return await inTx(outerTx, async (tx) => {
      ...
      await lockSortList({ tx, name: 'drugCategory' })
      await shiftDownForNew({ tx, table: tx.drugCategory, name: 'drugCategory' })

      const created = await tx.drugCategory.create({
...
export type UpdateDrugCategoryInput = { name: string }

/** แก้ชื่ออย่างเดียว — ลำดับเป็นงานของ `moveDrugCategory` */
export async function updateDrugCategory(
  id: bigint,
  input: UpdateDrugCategoryInput,
  actorId: bigint,
  outerTx?: Tx,
): Promise<DrugCategory> {
  const value = name(input.name)
  ...
  const existing = await (outerTx ?? db).drugCategory.findFirst({
  ...
  try {
    return await inTx(outerTx, async (tx) => {
      const updated = await tx.drugCategory.update({
        where: { id },
...
export async function deleteDrugCategory(
  ...
  const existing = await (outerTx ?? db).drugCategory.findUnique({ where: { id } })
  if (!existing) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })
  ...
  await inTx(outerTx, async (tx) => {
    ...
    await closeGap({ tx, table: tx.drugCategory, name: 'drugCategory' }, vacated)
...
export type MoveDrugCategoryInput = {
...
export async function moveDrugCategory(
  id: bigint,
  input: MoveDrugCategoryInput,
  actorId: bigint,
  outerTx?: Tx,
): Promise<void> {
  const at = outerTx ?? db

  const row = await at.drugCategory.findFirst({ where: { id, deletedAt: null } })
  ...
  const to = await resolveMoveTarget(at.drugCategory, {
    id,
    from,
  ...
  await inTx(outerTx, async (tx) => {
    // เข้าคิวก่อนทำงาน — เหตุผลอยู่ที่ `lockSortList`
    await lockSortList({ tx, name: 'drugCategory' })
    await shiftBetween({ tx, table: tx.drugCategory, name: 'drugCategory' }, { from, to })

    await tx.drugCategory.update({ where: { id }, data: { sortOrder: to, updatedBy: actorId } })
