...
/**
 * หมวดบริการ — ฝ่ายผลิต ฝ่ายขาย ฝ่ายบัญชี
 *
 * รูปเดียวกับทะเบียนอื่น ต่างตรงเดียว: **มีคนสังกัดอยู่** รายการรักษากับรายการรักษาชี้มาที่นี่
 * การลบจึงต้องถามถึงพวกเขาก่อน (ผู้ใช้ตัดสิน 2026-08-26 · ดู `///` บนหัว `ServiceCategory`)
 */

const MODULE = 'serviceCategory'
const LABEL = 'หมวดบริการ'
const NAME_MAX = 200

export const SERVICE_CATEGORY_LIST_LIMIT = 500
...
async function guardDelete(id: bigint, at: Tx | Db): Promise<void> {
  const children = await at.serviceItem.count({ where: { categoryId: id, deletedAt: null } })

  if (children > 0) {
    throw inUse(`ลบหมวดบริการนี้ไม่ได้ เพราะยังมีรายการรักษาสังกัดอยู่`, { children })
  }
}

/**
 * ลูกที่ถูกลบไปแล้วเลิกชี้มาที่หมวดบริการที่กำลังจะหายไป
 *
...
export type ListServiceCategorysInput = {
  q?: string | undefined
 ...
 */
export async function listServiceCategorys(
  input: ListServiceCategorysInput = {},
  tx?: Tx,
): Promise<ServiceCategory[]> {
  const q = input.q?.trim()
  const limit = Math.min(input.limit ?? SERVICE_CATEGORY_LIST_LIMIT, SERVICE_CATEGORY_LIST_LIMIT)

  return (tx ?? db).serviceCategory.findMany({
    where: {
      deletedAt: null,
...
export type CreateServiceCategoryInput = { name: string }

/**
 * เพิ่มรายการ — แถวใหม่ขึ้นบนสุดเสมอ
 *
 * ทั้งก้อนอยู่ในทรานแซกชันเดียว · วางแถวไว้ที่รายการรักษา 0 แปลว่าทุกแถวที่เหลือต้องเลื่อนลง
...
export async function createServiceCategory(
  ...
  try {
    return await inTx(outerTx, async (tx) => {
      ...
      await lockSortList({ tx, name: 'serviceCategory' })
      await shiftDownForNew({ tx, table: tx.serviceCategory, name: 'serviceCategory' })
...
export type UpdateServiceCategoryInput = { name: string }

/** แก้ชื่ออย่างเดียว — ลำดับเป็นงานของ `moveServiceCategory` */
export async function updateServiceCategory(
  ...
  const existing = await (outerTx ?? db).serviceCategory.findFirst({
...
export async function deleteServiceCategory(
  ...
  const existing = await (outerTx ?? db).serviceCategory.findUnique({ where: { id } })
  if (!existing) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })
  ...
  await inTx(outerTx, async (tx) => {
     ...
     * ทุกคนอ่านค่าเดิมชุดเดียวกัน แล้วต่างคนต่างปิดช่องจากรายการรักษาที่ไม่มีอยู่แล้ว
    ...
    await tx.serviceCategory.update({
      ...
      data: {
        ...
        //
        // null ไม่ใช่ตัวเลข: แถวไม่ได้อยู่ในลิสต์แล้ว จึงไม่ถือรายการรักษาในลิสต์
        sortOrder: null,
      },
    })

    await closeGap({ tx, table: tx.serviceCategory, name: 'serviceCategory' }, vacated)
...
export async function moveServiceCategory(
  id: bigint,
  ...
  const row = await at.serviceCategory.findFirst({ where: { id, deletedAt: null } })
  if (!row) throw notFound(`ไม่พบ${LABEL}นี้`, { id: String(id) })

  const from = row.sortOrder
  if (from === null) {
    // แถวที่ยังอยู่ต้องถือรายการรักษาเสมอ — มาถึงตรงนี้แปลว่าตารางไม่คงเส้นคงวา
    throw notFound(`${LABEL}นี้ไม่มีลำดับในลิสต์`, { id: String(id) })
  }

  const to = await resolveMoveTarget(at.serviceCategory, {
    id,
    from,
  ...
  await inTx(outerTx, async (tx) => {
    // เข้าคิวก่อนทำงาน — เหตุผลอยู่ที่ `lockSortList`
    await lockSortList({ tx, name: 'serviceCategory' })
    await shiftBetween({ tx, table: tx.serviceCategory, name: 'serviceCategory' }, { from, to })

    await tx.serviceCategory.update({ where: { id }, data: { sortOrder: to, updatedBy: actorId } })
