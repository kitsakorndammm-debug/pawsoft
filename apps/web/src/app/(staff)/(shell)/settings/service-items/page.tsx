...
import { ServiceCategoryManagerDialog } from '@/features/service-category/service-category-manager'
...
import { SERVICE_ITEM_PAGE_SIZE, useDeleteServiceItem, useServiceItems, useSetServiceItemActive } from '@/features/service-item/hooks'
...
const NOUN = 'รายการรักษา'

/**
 * รายการรักษาและบริการ
 *
 * **หมวดบริการจัดการจากในหน้านี้ ไม่มีเมนูแยก** (ผู้ใช้ตัดสิน 2026-09-01 · รูปเดียวกับ
 * หน้าลูกค้าของระบบก่อนหน้า) · คนที่มาเพิ่มรายการคือคนเดียวกับที่รู้ว่าต้องมีหมวดใหม่ ·
...
function ServiceItemList() {
  ...
  const { data, isPending, isFetching, isError, error } = useServiceItems(debounced.trim(), null, page)
  const categories = useServiceCategoryOptions()

  const setActive = useSetServiceItemActive()
  const remove = useDeleteServiceItem()

  const [dialog, setDialog] = useState<{ mode: DialogMode; row: ServiceItem | null; open: boolean }>({
    mode: 'create',
    row: null,
    open: false,
  })
  ...
  const openDialog = (mode: DialogMode, row: ServiceItem | null) => setDialog({ mode, row, open: true })
  ...
  const columns: DataTableColumn<ServiceItem>[] = [
    ...
    { key: 'name', title: `ชื่อ${NOUN}`, truncate: true, dataIndex: 'name' },
        { key: 'description', title: 'คำอธิบาย', truncate: true, dataIndex: 'description' },
    {
      key: 'categoryId',
      title: 'หมวด',
      width: 140,
    ...
    <div className="flex h-full flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {/* หมวดบริการอยู่ในกล่อง ไม่ใช่เมนูแยก — ดู `///` บนหัวไฟล์ */}
          <Button
            type="button"
            variant="outline"
      ...
      <DataTable<ServiceItem>
        ...
        pagination={{
          ...
          limit: data?.page.pageSize ?? SERVICE_ITEM_PAGE_SIZE,
      ...
      <ServiceCategoryManagerDialog open={categoriesOpen} onOpenChange={setCategoriesOpen} />
