...
import { useCreateServiceCategory, useUpdateServiceCategory } from '@/features/service-category/hooks'


/** ยาวสุดที่ BE รับ — ตรงกับ `NAME_MAX` ใน `apps/api/src/modules/service-category/service-category.service.ts` */
const NAME_MAX = 200
...
const schema = z.object({
  name: z
    .string()
    .trim()
    .min(1, 'กรอกชื่อหมวดบริการ')
    .max(NAME_MAX, `ยาวเกิน ${NAME_MAX} ตัวอักษร`),
  /**
   * ชนิดบังคับเลือก — ไม่มีค่ากลาง
   *
   * BE ให้ไม่ส่งมาได้ (แล้วมันใช้ค่าตั้งต้นของฐาน) แต่หน้าจอนี้ถามเสมอ · หมวดบริการ
...
const TITLE: Record<DialogMode, string> = {
  create: 'เพิ่มหมวดบริการ',
  edit: 'แก้ไขหมวดบริการ',
  view: 'หมวดบริการ',
}

/**
 * กล่องเพิ่ม · แก้ · ดู ของหมวดบริการ
 *
...
export function ServiceCategoryDialog({
  ...
  async function handleSubmit(values: FormValues): Promise<string> {
    ...
    if (mode === 'create') {
      ...
      return `เพิ่มหมวดบริการ "${saved.name}" แล้ว`
    }
