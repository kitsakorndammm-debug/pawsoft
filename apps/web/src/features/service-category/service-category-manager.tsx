...
import type { ServiceCategory } from '@/features/service-category/api'
import { ServiceCategoryDialog } from '@/features/service-category/service-category-dialog'
import {
  useServiceCategorys,
  useDeleteServiceCategory,
  useMoveServiceCategory,
} from '@/features/service-category/hooks'
import { toErrorMessage } from '@/lib/api-client'
import { MASTER_WRITE } from '@/lib/permissions'
import { useDebounce } from '@/lib/use-debounce'

/**
 * จัดการหมวดบริการ — **อยู่ในกล่อง ไม่ใช่หน้าของตัวเอง** (ผู้ใช้ตัดสิน 2026-08-27)
 *
 * เคยเป็นหน้า `/settings/service-categorys` · ย้ายมาเพราะคนที่ต้องแก้ทะเบียนนี้กำลังกรอก
 ...
 */
const NOUN = 'หมวดบริการ'

export function ServiceCategoryManagerDialog({
  open,
  onOpenChange,
}: {
  open: boolean
  ...
  const { data: rows, isPending, isFetching, isError, error } = useServiceCategorys(debouncedSearch)
  ...
  function openDialog(mode: DialogMode, row: ServiceCategory | null) {
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex h-[80vh] max-w-3xl flex-col gap-3">
          ...
          <SortableDataTable<ServiceCategory>
